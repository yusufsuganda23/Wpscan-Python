...
if __name__ == "__main__":
    ...
    parser.add_argument('--aggressive', action='store_const', const='aggressive', dest='aggressive', default=False,
                        help="Aggressive scan for plugins/themes")
    parser.add_argument('--fuzz', action='store_const', const='fuzz', dest='fuzz', default=False, help="Fuzz the files")
    parser.add_argument('--brute', action='store_const', const='brute', dest='brute', default=False,
                        help="Bruteforce users and passwords")
    parser.add_argument('--nocheck', action='store_const', const='nocheck', dest='nocheck', default=False,
                        help="Check for a Wordpress instance")
    parser.add_argument('--random-agent', action='store_const', const='random_agent', dest='random_agent',
                        default=False, help="Random User-Agent")
    parser.add_argument('--threads', action='store', dest='max_threads', default=1, help="Number of threads to use")
    parser.add_argument('--usernames', action='store', dest='usernames', default='',
                        help="Usernames to bruteforce separated with a ','")
    parser.add_argument('--users-list', action='store', dest='users_list', default=None,
                        help="Users list for bruteforce")
    parser.add_argument('--passwords-list', action='store', dest='passwords_list', default=None,
                        help="Passwords list for bruteforce")
    parser.add_argument('--debug', action='store_const', const='debug', dest='debug', default=False,
                        help="Enable a debugging flag")
    ...
    if results.url is not None:
        ...
        api_token = ""  # Flushes the value of api_token
        print(info("You can find your API here: https://wpscan.com/profile"))
        api_token = input("Enter your API token: ")

        # Check if the API token is empty
        if not api_token:
            print("API token cannot be empty. Please provide a valid token.")
            exit(1)  # Exit the program with a non-zero status code to indicate an error

        # Build a new wordpress object
        wp = Wordpress(format_url(results.url), results.random_agent, results.nocheck, results.max_threads, api_token)
        ...
        # Launch scans
        Scan_Engine(wp, results.aggressive, api_token)
