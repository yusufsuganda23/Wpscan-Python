...
class Wordpress:
    ...
    def __init__(self, url, user_agent, nocheck, max_threads, api_token):
        ...
        try:
            ...
            if 'location' in r.headers:
                ...
                if "wp-admin/install.php" in r.headers['location']:
                    print(critical(
                        "The Website is not fully configured and currently in install mode. Call it to create a new "
                        "admin user."))
                    exit()
                # Redirect
                print(notice("The remote host tried to redirect to: %s" % r.headers['location']))
                user_input = input("[?] Do you want to follow the redirection ? [Y]es [N]o, ")
    ...
    def is_readme(self):
        ...
        if "200" in str(r):
            ...
            if len(matches) > 0 and matches[0] is not None and matches[0] != "":
                ...
                print(critical(
                    "The Wordpress '%s' file exposing a version number: %s" % (self.url + 'readme.html', matches[0])))
    ...
    def full_path_disclosure(self):
        r = requests.get(self.url + "wp-includes/rss-functions.php", headers={"User-Agent": self.agent},
                         verify=False).text
        regex = re.compile("Fatal error:.*? in (.*?) on", re.S)
        matches = regex.findall(r)

        if matches:
            print(warning("Full Path Disclosure (FPD) in %s exposing %s" % (
                self.url + "wp-includes/rss-functions.php", matches[0].replace('\n', ''))))
                ...
                try:
                    ...
                    if isinstance(users, list):
                        ...
                        for user in users:
                            ...
                            if user_id and user_name and user_slug:
                                print(info(
                                    "\tIdentified the following user: %s, %s, %s" % (user_id, user_name, user_slug)))
                            else:
                                print(warning("Incomplete user information found: %s" % user))
                    else:
                        print(warning("Invalid user data format. Expected a list."))

                except json.JSONDecodeError as e:
                    print(warning("JSON decoding failed while enumerating WordPress users: %s" % str(e)))

            else:
                print(warning("Unable to enumerate WordPress users. Status code: %d" % r.status_code))

        except requests.RequestException as e:
            print(warning("Error occurred while enumerating WordPress users: %s" % str(e)))
